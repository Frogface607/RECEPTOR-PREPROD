#!/usr/bin/env python3
"""
Article Generation Fix Testing
Тестирование исправления генерации артикулов в LLM pipeline.

ЦЕЛЬ: Проверить, что после исправления LLM pipeline:
1. Техкарты генерируются с артикулами для блюда и ингредиентов
2. Экспорт работает корректно с реальными артикулами
3. Скелетоны не пустые и содержат правильные данные
"""
...
class ArticleGenerationTester:
    ...
    async def test_7_end_to_end_workflow(self):
        ...
        try:
            ...
            second_techcard_id = response.json().get('id')
