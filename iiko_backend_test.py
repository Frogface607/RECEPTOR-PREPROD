...
def main():
    ...
    try:
        ...
        if organizations:
            print("✅ УСПЕХ! JWT токен работает - интеграция с IIKo заработала!")
            print("✅ Организации найдены - можно загружать AI-техкарты в POS-систему!")
            print("🎉 ЭТО РЕШАЮЩИЙ МОМЕНТ - ИНТЕГРАЦИЯ НАКОНЕЦ ЗАРАБОТАЛА!")
        else:
            print("❌ JWT токен пока не дает доступ к организациям")
            print("⚠️ Требуется дополнительная настройка или проверка токена")
