...
async def analyze_finances(request: dict):
    ...
    prompt = f"""Ты — топовый финансовый аналитик ресторанного бизнеса с 15-летним опытом работы с премиум-заведениями.

Проанализируй финансовые показатели блюда "{dish_name}" максимально ДЕТАЛЬНО и ПРОФЕССИОНАЛЬНО.
...
ВАЖНО: 
- ВСЕ ЦИФРЫ должны быть ТОЧНЫМИ и РЕАЛИСТИЧНЫМИ
- Учитывай региональный коэффициент {regional_coefficient}x
- Используй реальные цены российского рынка 2024 + инфляция 12%
- Давай практические советы для реального ресторана
- Давай КОНКРЕТНЫЕ советы с цифрами, а не общие фразы
- Указывай точные суммы экономии
- Анализируй каждый ингредиент отдельно
