/**
 * RECEPTOR Tools — catalog of 27 AI instruments for restaurant professionals.
 *
 * Ported verbatim from v1 (`legacy-v1:frontend/src/data/tools-catalog.js`),
 * preserving the carefully-written Russian prompts. Typed for v2 and consumed
 * by the `/tools` showcase + `/api/tools/run`.
 *
 * Each tool is prompt-based: 1–2 fields → `buildPrompt(values)` → an LLM call.
 * On mock (no ANTHROPIC_API_KEY) the prompt feeds the deterministic
 * `mock-runner`; with a key it goes to Claude.
 *
 * `icon` values are lucide-react component names, resolved in the UI.
 */
...
export const TOOLS: Tool[] = [
  // ═══════════ ШЕФ-ПОВАР ═══════════
  {
    ...
    description:
      "Создаёт полный рецепт с ингредиентами, шагами приготовления и подачей",
    icon: "Utensils",
    fields: [
...
Формат: markdown.`,
  ...
  // ═══════════ ОФИЦИАНТЫ ═══════════
  {
    ...
    name: "Скрипт продаж",
    description:
      "Создаёт скрипт для официанта: приветствие, допродажи, работа с возражениями",
    icon: "MessageSquare",
    fields: [
...
Формат: markdown.`,
  ...
  // ═══════════ МАРКЕТИНГ ═══════════
  {
    ...
    name: "Ответ на отзыв",
    description:
      "Генерирует профессиональный ответ на отзыв гостя (позитивный или негативный)",
    icon: "MessageCircle",
    fields: [
...
Формат: готовый к публикации текст.`,
  ...
  },
  {
    ...
    icon: "Scale",
    fields: [
      { id: "ad_text", label: "Текст рекламы", placeholder: "Ужин с авторской кухней и сезонным меню. Специальное предложение только сегодня.", required: true, multiline: true },
    ],
    buildPrompt: (f) =>
...
Формат: markdown.`,
  ...
  },
  {
    ...
    name: "Идея акции",
    description:
      "Генерирует идеи акций, спецпредложений и маркетинговых активностей",
    icon: "Sparkles",
    fields: [
...
Формат: markdown.`,
  ...
  // ═══════════ HR & ПЕРСОНАЛ ═══════════
  {
    ...
    description:
      "Создаёт привлекательное описание вакансии для повара, бармена, официанта",
    icon: "Briefcase",
    fields: [
...
Формат: markdown.`,
  ...
  // ═══════════ ДОПОЛНИТЕЛЬНЫЕ ИНСТРУМЕНТЫ ═══════════
  {
    ...
    description:
      "Создаёт аппетитные описания блюд для печатного или электронного меню",
    icon: "FileText",
    fields: [
  ...
  },
  {
    ...
    description:
      "Рекомендует вина и напитки к блюдам для винной карты или официантов",
    icon: "Wine",
    fields: [
  ...
  },
  {
    ...
    description:
      "Анализирует расходы ресторана и предлагает конкретные способы экономии",
    icon: "TrendingDown",
    fields: [
  ...
  },
  {
    ...
    name: "Сезонное меню",
    description:
      "Предлагает идеи сезонных блюд на основе текущего месяца и доступных продуктов",
    icon: "Leaf",
    fields: [
...
export const getToolsByCategory = (categoryId: ToolCategoryId): Tool[] =>
  TOOLS.filter((t) => t.category === categoryId);

export const getFreeTools = (): Tool[] => TOOLS.filter((t) => t.free);
