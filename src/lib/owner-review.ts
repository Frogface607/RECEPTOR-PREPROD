...
function fieldContextTaskFor(kind: TeamFieldSignal["kind"]): {
  ...
  if (kind === "training") {
    return {
      title: "Закрыть пробел в знаниях команды",
      question:
        "какого знания или стандарта не хватает команде, чтобы завтра сделать правильно",
      checklistTitle: "Разбор: факт, вопрос, проверка, действие",
      check:
        "Собрать один конкретный пробел: что сотрудники не понимают, в какой ситуации это проявилось и какой короткий стандарт или урок нужно дать перед следующей сменой.",
      learningModuleId: "shift-brief",
      learningModuleTitle: "Брифинг смены и передача контекста",
    };
  }
  ...
  if (kind === "shift") {
    return {
      ...
      checklistTitle: "После смены собери итог смены",
      check:
        "Разобрать итог смены по четырем пунктам: посадка и гости, что повлияло на выручку, что заметила команда и какое одно действие попадет в утренний бриф.",
      learningModuleId: "shift-brief",
      learningModuleTitle: "Брифинг смены и передача контекста",
    };
  }
  if (kind === "weather") {
    return {
      title: "Связать погоду и брони с результатом смены",
      question:
        "как погода, отмены или поток гостей повлияли на смену и что подготовить в следующий похожий день",
      checklistTitle: "Если BI показал слабую смену",
      check:
        "Сверить погоду, отмены броней, посадку, состав смены и продажи по времени, чтобы отличить рыночный фактор от ошибки команды.",
