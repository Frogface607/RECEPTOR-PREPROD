...
export function OnboardingWizard({ demoMode }: { demoMode: boolean }) {
  ...
  const finish = () => {
    ...
    startTransition(async () => {
      const res = await createVenueAction({ name, type, city });
      if (!res.ok) {
        setError(res.error);
        return;
      }
        ...
        {step === 0 ? (
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="rounded-lg border border-border/50 bg-background/40 p-4 text-[13px] leading-relaxed text-muted-foreground">
              Нет ключа под рукой? Пропустите — покажем дашборд на демо-данных
              Edison, подключите iiko позже в настройках.
            </div>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-4 py-6 text-center">
            ...
            <p className="max-w-sm text-[14px] leading-relaxed text-muted-foreground">
              {demoMode
                ...
                : "Создадим заведение и откроем дашборд. Дальше подключим живые данные iiko."}
            </p>
            {error ? (
