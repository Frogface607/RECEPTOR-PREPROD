...
function App() {
  ...
  const formatTechCard = (content) => {
    ...
    // ПРОСТОЕ РЕШЕНИЕ - ВСЕГДА ПОКАЗЫВАЕМ ИНГРЕДИЕНТЫ В НАЧАЛЕ!
    const allIngredientLines = lines.filter(l => 
      l.startsWith('- ') && (l.includes('₽') || l.includes('руб'))
    );
    
    console.log('ALL INGREDIENTS FOUND:', allIngredientLines);
    
    if (allIngredientLines.length > 0) {
      result.push(
        <div key="always-ingredients" className="my-8">
    ...
    for (let index = 0; index < lines.length; index++) {
      ...
      // SKIP ingredients section - мы уже показали их выше
